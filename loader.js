...
(function(){
	...
	function delayProgress(delayMS) {
        ...
        async function tryDownloadClient(ipfsCID, ipfsPath) {
    		// The file you want to load is in the same folder as loader.js
  		// Use opts.cid as the base filename for your JavaScript file
   		let localFilePath = `Minecraft_Scratch_Loader_Payload.js`; // Replace opts.cid with the actual CID you want to use (if it's not being used already)

    		// Construct the URL based on the current location of loader.js
   		let currentScriptDir = document.currentScript.src.replace(/\/[^\/]*$/, '/'); // Get the directory of the current script (loader.js)
    		let fileURL = currentScriptDir + localFilePath;

    		try {
        		updateProgressBar("Loading client from local storage...", fileURL, 0.0);

        		// Fetch the .js file from the same folder as loader.js
        		let clientFile = await fetch(fileURL);

        		if (clientFile.ok) {
            			let clientText = await clientFile.text();  // Read the file content as text
					...
					// No decompression needed if not gzipped
            			...
            			// You can evaluate the JavaScript code or pass it to another function to run
            			return clientText;
        		} else {
            			throw `Failed to fetch local client file from: ${fileURL}`;
        		}
    		} catch (error) {
        		console.error("Failed to load local client file:", error);
        		updateProgressBar("Client load failed!", fileURL, -1);
        		return null;
    		}
	}
	...
	async function tryDownloadClient(ipfsCID, ipfsPath) {
    		// The file you want to load is in the same folder as loader.js
    		let localFilePath = `${opts.cid}.js`; // Replace opts.cid with the actual CID you want to use (if it's not being used already)

    		// Get all script tags in the document
    		let scripts = document.getElementsByTagName('script');
   		let currentScriptDir = '';

    		// Loop through the script tags to find the loader.js script
    		for (let i = 0; i < scripts.length; i++) {
        		let script = scripts[i];
        		if (script.src && script.src.includes('loader.js')) {
            			currentScriptDir = script.src.replace(/\/[^\/]*$/, '/'); // Extract the directory path of loader.js
            			break;
        		}
    		}

    		// If we couldn't find the script's directory, log an error
    		if (!currentScriptDir) {
        		console.error("Unable to determine the path of loader.js");
        		return null;
    		}

    		// Construct the full file URL by appending the local file name
    		let fileURL = currentScriptDir + localFilePath;
